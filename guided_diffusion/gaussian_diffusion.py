...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        pbar = tqdm(list(range(self.num_timesteps))[::-1], leave=False, delay=0.1)
        for idx in pbar:
            ...
            beta_t = _extract_into_tensor(self.betas, time, img.shape)

            # TODO: how can we handle argument for different condition method?
            operator = kwargs.get('operator')
            current_rho = (self.sqrt_alphas_cumprod[idx] ** 2) / (self.sqrt_one_minus_alphas_cumprod[idx] ** 2 + 1e-4)
            sigmas_all = self.sqrt_one_minus_alphas_cumprod
            recon_method = kwargs.get('recon_method')
            real_alpha = kwargs.get('real_alpha')
            # This is the specific sigma for the current step
            ...
            if record:
                if idx % 50 == 0:
                    ...
                    saved_img = clear_color(img)

                    # If the image is (:, :, 1), squeeze it to (:, :)
                    if saved_img.ndim == 3 and saved_img.shape[-1] == 1:
                        saved_img = saved_img.squeeze(-1)

                    # Save with a gray colormap
                    plt.imsave(file_path, saved_img, cmap='gray')
