...
def normalize_np(img, gamma=1.0):
    # """ Normalize img in arbitrary range to [0, 1] """
    # if np.max(img) == np.min(img):
    #     print('Image has constant value: ' + str(np.max(img)))
    #     return img
    # if np.isnan(img).any():
    #     print('Image has contains nan')
    #     return img
    # if np.isinf(img).any():
    #     print('Image has contains inf')
    #     return img
    # img -= np.min(img)
    # img /= np.max(img)
    # return img
    """
    Normalizes image to [0, 1] and applies gamma correction.
    Supports both NumPy arrays and PyTorch Tensors (CPU/CUDA).
    """
    # --- 1. Identify the library ---
    if torch.is_tensor(img):
        lib = torch
        # Helper to check for non-finite values
        if not lib.isfinite(img).all():
            print("Image contains NaN or Inf")
            return img
    else:
        lib = np
        if not lib.isfinite(img).all():
    ...
    if img_max == img_min:
        print(f"Image has constant value: {img_max}")
        return img

    # --- 3. Scale and Gamma Correct ---
    img = (img - img_min) / (img_max - img_min)

    if gamma != 1.0:
        if lib == torch:
            # Clamp to prevent tiny precision errors (e.g. -1e-7) from breaking pow
            img = lib.pow(lib.clamp(img, min=0.0), gamma)
        else:
            img = lib.power(img, gamma)

    return img


def normalize_percentile_np(img, p=99.0):
    """
    Normalize img to [0, 1] using the p-th percentile as the max
    to handle high-flux outliers in SPAD reconstructions.
    """
    if torch.is_tensor(img):
        img = img.clone().detach().cpu().numpy()
    else:
        img = img.copy()  # Original numpy logic


    v_min = np.min(img)
    v_max = np.percentile(img, p)

    # Clip values above the percentile to the percentile value
    # and values below the minimum (usually 0) to v_min
    img = np.clip(img, v_min, v_max)

    # Standard normalization logic from your blueprint
    img -= v_min
    if (v_max - v_min) > 0:
        img /= (v_max - v_min)
